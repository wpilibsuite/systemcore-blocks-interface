...
# Prevent BSD tar from embedding AppleDouble (._*) files and extended attributes.
# This env var is a no-op on Linux.
...
# Strip macOS metadata files (no-op on Linux)
find "$BUILD_DIR" \( -name ".DS_Store" -o -name "._*" \) -delete

echo "Building IPK dir structure"
cd "$BUILD_DIR"

# GNU tar uses --owner/--group; BSD tar (macOS) uses --uid/--gid/--uname/--gname.
