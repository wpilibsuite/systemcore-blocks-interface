// This file was generated. Do not edit!

import * as python from "../python.js"

// Blocks for class wpilib.PneumaticHub.StickyFaults

python.PythonVariableGetterNames["instance wpilib.PneumaticHub.StickyFaults int"] = ["Brownout", "CanBusOff", "CanWarning", "CompressorOpen", "CompressorOverCurrent", "HasReset", "SolenoidOverCurrent"];
python.PythonVariableGetterTooltips["instance wpilib.PneumaticHub.StickyFaults int"] = ["", "", "", "", "", "", ""];
python.PythonVariableSetterNames["instance wpilib.PneumaticHub.StickyFaults int"] = ["Brownout", "CanBusOff", "CanWarning", "CompressorOpen", "CompressorOverCurrent", "HasReset", "SolenoidOverCurrent"];
python.PythonVariableSetterTooltips["instance wpilib.PneumaticHub.StickyFaults int"] = ["", "", "", "", "", "", ""];


function getToolboxCategory(subcategories) {
  const category = {
    kind: "category",
    name: "StickyFaults",
    contents: [
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "Brownout"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "Brownout"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CanBusOff"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CanBusOff"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CanWarning"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CanWarning"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CompressorOpen"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CompressorOpen"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CompressorOverCurrent"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "CompressorOverCurrent"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "HasReset"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "HasReset"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "get_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "SolenoidOverCurrent"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "set_python_instance_variable", "extraState": {"varType": "int", "key": "instance wpilib.PneumaticHub.StickyFaults int", "importModule": "", "selfLabel": "stickyFaults", "selfType": "wpilib.PneumaticHub.StickyFaults"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults", "VAR": "SolenoidOverCurrent"}, "inputs": {"SELF": {"block": {"type": "variables_get", "fields": {"VAR": {"name": "myStickyFaults"}}}}}},
      {"kind": "block", "type": "variables_set", "fields": {"VAR": {"name": "myStickyFaults"}}, "inputs": {"VALUE": {"block": {"kind": "block", "type": "call_python_constructor", "extraState": {"tooltip": "", "returnType": "wpilib._wpilib.PneumaticHub.StickyFaults", "args": [], "importModule": "wpilib"}, "fields": {"CLASS": "wpilib.PneumaticHub.StickyFaults"}}}}},
    ],
  };
  if (subcategories) {
    category.contents.push(...subcategories);
  }
  return category;
}

export {getToolboxCategory}
